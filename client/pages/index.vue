<template>
  ...
  <div>
    ...
    <div v-if="step === 1">
      ...
      <div class="custom-input-container">
        <InputText class="custom-input" style="margin-left: 40px;" v-model="formData.destination"
          placeholder="도시 이름을 입력해 주세요." @keyup.enter="nextStep" />
        <Button class="custom-button" :disabled="!formData.destination" @click="nextStep">다음</Button>
      </div>
    </div>
    ...
    <div v-if="step === 2">
      <div class="header-area">
        <i class="pi pi-angle-left prev-step-icon" @click="prevStep"></i>
      </div>
      <h2 class="step-title">여행 일정을 알려주세요.</h2>
      <div class="custom-input-container">
        <Calendar class="calendar-input" v-model="formData.start" showButtonBar dateFormat="yy-mm-dd"
          placeholder="출발일" />
        <p style="text-align: center; line-height: 3; width:5%; font-weight: 600;">-</p>
        <Calendar class="calendar-input" v-model="formData.end" showButtonBar dateFormat="yy-mm-dd" placeholder="도착일" />
        <Button class="custom-button" :disabled="!formData.start || !formData.end" @click="nextStep">다음</Button>
      </div>
    </div>
    ...
    <div v-if="step === 3">
      ...
      <div class="custom-input-container">
        <div class="input-wrapper">
          <InputText v-model="spot" class="custom-input" @keyup.enter="searchSpot" placeholder="장소명을 입력해 주세요."
            @focus="isFocused = true" @blur="isFocused = false" />
          <i class="pi pi-arrow-right search-icon" :class="{ 'icon-activate': isFocused }" @click="searchSpot"></i>
        </div>
        <div class="input-wrapper">
          <Chip v-for="spot in formData.spots" :key="spot.name" :label="spot.name" removable @remove="removeSpot(spot)"
            class="chip-item" />
        </div>
  ...
  <div v-if="step === 4">
    ...
    <div class="input-wrapper">
      <h2 class="step-title" style="margin-left: 0px;">여행 시작 장소를 알려주세요.</h2>
      <InputText v-model="arrival" class="custom-input" :class="{ 'input-activate': arrival || isArrivalFocused }"
        @focus="isArrivalFocused = true" @blur="isArrivalFocused = false" @keyup.enter="searchArrival"
        placeholder="여행지 내에서 여행을 시작할 장소   EX:강릉역" />
      <i class="pi pi-arrow-right search-icon" style="top:70%" :class="{ 'icon-activate': isArrivalFocused }"
        @click="searchSpot"></i>
    </div>
    <div class="input-wrapper">
      <h2 class="step-title" style="margin-left: 0px;">여행 종료 장소를 알려주세요.</h2>
      <InputText v-model="depart" class="custom-input" :class="{ 'input-activate': depart || isDepartFocused }"
        @focus="isDepartFocused = true" @blur="isDepartFocused = false" @keyup.enter="searchDepart"
        placeholder="여행지 내에서 여행을 종료할 장소   EX:강릉역" />
      <i class="pi pi-arrow-right search-icon" style="top:70%" :class="{ 'icon-activate': isDepartFocused }"
        @click="searchSpot"></i>
    </div>

    <Button class="custom-button" :disabled="!formData.depart || !formData.arrival" @click="submitForm">경로 검색</Button>
  </div>
...
const recommandLoading = ref(false)
const recommandationData = ref([])
async function recomandation(spot: placeItem) {
  console.log(spot)
  try {
...
.custom-input-container {
  ...
  /* 요소들 간의 공간을 균등하게 분배 */
  flex-wrap: wrap;
  /* 화면이 좁을 때 컴포넌트들이 자동으로 다음 줄로 내려가도록 */
}

.calendar-input {
  flex-grow: 1;
  /* 각 Calendar가 동일한 비율로 크기를 차지 */
  max-width: 45%;
  /* Calendar의 최대 너비 설정 */
  height: 44px;
  box-sizing: border-box;
  /* 요소가 부모 요소의 크기를 초과하지 않도록 */
  padding-inline: 20px;
  text-align: center;
  margin-inline: auto;
}

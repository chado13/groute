<template>
    ...
    <div v-if="step === 3">
      ...
      <div class="custom-input-container">
        ...
        <div
          ...
          v-if="lastChipItem.trim() !== '' && spot.trim() === ''"
        >
          <p style="margin: 20px 20px 0 20px">
            <span style="color: #3dac68; font-weight: 500; font-size: large">{{
              lastChipItem
            }}</span>
            근처 여긴 어때요?
          </p>
  ...
  <div v-if="step === 4">
    ...
    <div class="custom-input-container">
      <h2 class="step-title" style="margin-top: 50px">
        여행 종료 장소를 알려주세요.
      </h2>
      <div class="input-wrapper">
        <InputText
  ...
  <div v-if="step === 5">
    ...
    <div style="width: 100%; height: 48px; top: 20px">
      <i
        class="pi pi-angle-left"
        style="font-size: 1.8rem; line-height: 2; margin-left: 16px"
        @click="prevStep"
      ></i>
    </div>
...
watch(
  ...
  (newDestination, oldDestination) => {
    if (newDestination !== oldDestination) {
      formData.value.spots = [];
      spot.value = "";
      lastChipItem.value = "";
      recommandationData.value = [];
    }
  }
);
...
function removeSpot(spot: string) {
  ...
  if (index !== -1) {
    ...
    if (spot.name === lastChipItem.value) {
      lastChipItem.value = "";
      recommandationData.value = [];
    }
  }
